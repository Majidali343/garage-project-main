...
function Employeetask() {
  ...
  const downloadPDF = (id) => {
    axios
      .get(`http://77.37.49.209:5000/invoice/getpdf/${id}`, {
        responseType: "blob",
      })
      .then((response) => {
  ...
  const handleSubmit = async () => {
    const formData = {
      ...
      location,
      dates,
      amounts, // Make sure amounts is included in the submission
      advances,
      pendings,
      projectStatuses,
      invoices_num,
    ...
    try {
      const result = await fetch(
        "http://77.37.49.209:5000/invoice/post/E-invoice",
        {
          method: "POST",
          headers: {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        "http://77.37.49.209:5000/invoice/get/E-invoice"
      );
                        ...
                        <div className="absolute mt-2 w-full py-1 bg-white border border-gray-300 rounded shadow-lg">
                          {[
                            ...
                            <button
                              key={item}
                              onClick={() =>
                                handleDropdownChange(item, 0, "vehicle")
                              }
                        ...
                        <div className="absolute mt-2 w-full py-1 bg-white border border-gray-300 rounded shadow-lg">
                          {["Pending", "Complete"].map((status) => (
                            <button
                              key={status}
                              onClick={() =>
                                handleDropdownChange(status, 0, "projectStatus")
                              }
                              className="block w-full text-left px-4 py-2 text-gray-700 hover:bg-gray-200"
                            >
                              {status}
                ...
                {filteredData.map((invoice, index) => (
                  <tr key={index} className="border-t">
                    ...
                    <td className="py-3 px-6 text-center text-xs">
                      {" "}
                      <button
                        className="bg-[#ea8732] p-1 rounded-md text-white font-medium"
